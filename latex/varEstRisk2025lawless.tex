...
\title{Quantification of diagnostic probability in genomic interpretation}
...
The framework replaces categorical variant classification with continuous posterior probabilities that capture residual uncertainty from incomplete or missing genotype data. Demonstrations in three diagnostic scenarios show accurate quantification of variant-level disease relevance. 
% Application to 557 genes implicated in inborn errors of immunity (IEI) generated a public database of prior probabilities. Integration with protein-protein interaction and immunophenotypic data revealed gene-level constraint patterns, and validation in national cohorts showed close agreement between predicted and observed case numbers.
Application to a curated gene-disease panel generated calibrated prior probabilities and posterior diagnostic estimates. Validation in dominant and recessive disease cohorts showed agreement between predicted and observed case counts.
\vspace{1em}
...
In this study, we derive genome-wide prior probabilities of disease-associated variants and integrate them into a Bayesian diagnostic model. We demonstrate the approach in a curated gene-disease panel and validate predicted incidence against national cohorts
\cite{poli_human_2025, lawless_panelapprex_2025, martin_panelapp_2019, poli_human_2025, bousfiha_2024_2025}.
...
We demonstrate the approach using a curated gene-disease panel comprising 557 genes.
...
Bayesian integration of all annotated alleles produced quantitative \ac{cri}s for pathogenic attribution that (i) preserve Hardy-Weinberg expectations, (ii) accommodate \ac{ad}, \ac{ar}, and \ac{xl} inheritance, and (iii) explicitly quantify uncertainty from unsequenced, missing, or failed-\ac{qc} sites.
...
In scenario two, the prior distribution (Fig.~\ref{fig:plot_scenario_2_quant_uncert_ci}A) is first partitioned by observed versus missing alleles and weighted by clinical evidence. Posterior normalisation (Fig.~\ref{fig:plot_scenario_2_quant_uncert_ci}B–C) then redistributes probability mass across competing causal hypotheses, concentrating support on the confirmed nonsense variant \texttt{p.Ser237Ter} and the unsequenced splice-donor \texttt{c.159+1G{\small\textgreater}A}. Restricting to causal candidates (Fig.~\ref{fig:plot_scenario_2_quant_uncert_ci}D) makes explicit that diagnostic confidence is shared between these two alleles.

The posterior decomposition (Fig.~\ref{fig:plot_scenario_2_quant_uncert_ci}E) separates observed and missing contributions, yielding 40\% and 34\% of the damaging probability, respectively. The resulting gene-level estimate (Fig.~\ref{fig:plot_scenario_2_quant_uncert_ci}F) is a median diagnostic probability of 0.542 (95\% \ac{cri}: 0.264–0.8), reflecting residual uncertainty attributable to the unconfirmed alternative allele.
...
A third scenario with no observed variants is shown in supplemental data.
...
We found no evidence that functional pathogenicity scores correlate with variant occurrence probability, indicating that functional effect and population occurrence represent distinct probability dimensions (\textbf{Figure~\ref{fig:alphamissense_kw}}). 
...
% 0. Deficiencies
We must point out the most commonly described errors - people say what if a variant is unknown significance, or the model shown does not account for large structural variants or that a variant might cause one AD disease and a different opposite AR disease in another scenario - the reader must remember that the reference frame for each scenario is fixed only within that scenarion - the genome-wide scores for every variant are only for that single posterio calculate of that variant given all evidence and all other counterfactual evidence. the very same variant then recalcualted in a different scenario (another gene-disease panel, or effect hypothesis thus has a second set of results). To demonstrate every scenario here is impossible, and such teh method should not be dismissed at the whims of a simply undemonstrated scenario. 
...
% We demonstrated genome-wide gene distribution and \ac{moi} for the \ac{iei} panel in relation to \ac{ld}, confirming both feasibility and relevance. 
% However, 
Accurate \ac{ld} estimation requires population-specific, genome-wide pairwise genotype data. Our use of global reference allele frequencies improves generalisability but may reduce precision relative to population-specific estimates.
...
Estimating genetic disease risk is challenging because both penetrance and case attribution are uncertain \cite{zschocke_mendelian_2023}. Let \(P(D)\) denote the population risk of disease, \(P(G)\) the frequency of a genotype, \(P(D \mid G)\) the penetrance (the probability of disease given that genotype), and \(P(G \mid D)\) the proportion of affected individuals who carry that genotype. Disease risk can be written as
\[
P(D) = \frac{P(G)\,P(D \mid G)}{P(G \mid D)}.
\]
This expression shows that disease frequency depends not only on how common a genotype is, but also on how strongly it causes disease and how many cases it explains.

In practice, neither \(P(D \mid G)\) nor \(P(G \mid D)\) is known precisely \cite{minikel_quantifying_2016, whiffin_using_2017}. Both can vary across populations and clinical settings. Rather than fixing these quantities to single values, our method builds genome-wide prior probabilities using curated gene-disease associations, allele frequencies, and variant classifications. 
...
Although estimates for individual variants may be noisy, combining information across many variants makes the overall prior more stable through shrinkage \cite{efron_steins_1973}.

In curated gene panels, most cases are typically explained by a limited set of plausible genotypes. This corresponds to the practical assumption
\[
\sum_{i \in \mathcal{V}} P(G_i \mid D) \approx 1,
\]
meaning that the selected variant set accounts for nearly all cases. If this assumption is imperfect, the model reflects that uncertainty by widening posterior credible intervals rather than producing unstable point estimates. These priors therefore provide a quantitative basis for estimating disease probability and diagnostic confidence through Bayesian integration.


% By aggregating variant sets beyond single-gene analyses, the framework complements established rare variant association tests such as \ac{skat} and \ac{acat} \cite{liu2019acat,li2020dynamic,wu2011rare,lee2012optimal} and aligns with multi-omics integration methods \cite{kong2018nature,howe2021within}. 
It is compatible with \ac{acmg} guidelines \cite{richards2015standards}, related interpretation frameworks \cite{tavtigian2020fitting,li2017intervar}, and current \ac{qc} standards \cite{pedersen2021effective,anderson2010data}. Standardised systems, including \ac{acmg} Secondary Findings v3.2 \cite{miller2023acmg}, allow integration of these probabilistic results into clinical workflows.
Comparison with AlphaMissense scores showed that pathogenicity predictors estimate molecular effect rather than probability of occurrence. 
%Integrating variant-level priors with predictors like AlphaMissense could improve disease relevance by incorporating gene-disease associations and \ac{moi} for applications in AI/ML.
...
We used definitions from the \ac{iuis} \ac{iei} for the major disease categories used in this study \cite{poli_human_2025}.
...
Our computational pipeline estimated the probability of observing a disease-associated genotype for each variant.
For reporting we additionally aggregated these probabilities by gene and ClinVar classification. This approach included all variant classifications, not limited solely to those deemed ``pathogenic'', and explicitly conditioned the classification on the given phenotype, recognising that a variant could only be considered pathogenic relative to a defined clinical context. The core calculations proceeded as follows:

\subsubsection{Core probabilistic model}
\paragraph{Allele frequency and total variant frequency.} 
\label{sec:min_risk}
...
% HIDDEN COMMENT: IMPORTANT IMPLEMENTATION NOTE - DO NOT MODIFY THIS BLOCK:
% The use of p_i (allele frequency) instead of 2p_i (carrier frequency) is deliberate.
% This model defines priors at the allele level and incorporates diploid structure later.
% Do NOT replace p_i with 2p_i unless the entire Beta-Binomial and genotype-presence
% components are rederived accordingly. Changing this term alone will invalidate
% probabilistic calibration and downstream posterior interpretation.
\paragraph{Occurrence probability based on \ac{moi}.}
The probability that a variant contributes to disease depends on the \ac{moi}. For a variant \(i\) with allele frequency \(p_i\), we define its prior contribution \(p_{\text{disease},i}\).
For \textbf{\ac{ad}} and \textbf{\ac{xl}} conditions, a single pathogenic allele suffices. We therefore set
\(p_{\text{disease},i} = p_i\).
Here \(p_i\) denotes the population allele frequency. Under Hardy-Weinberg equilibrium, the carrier genotype frequency of a rare dominant allele is approximately \(2p_i\); however, because the framework defines priors at the allele level and incorporates diploidy in later steps, we use \(p_i\) directly.

For \textbf{\ac{ar}} conditions, two pathogenic alleles are required. Let \(P_{\text{tot}} = \sum_i p_i\)
denote the total allele frequency across pathogenic variants in the gene. Under Hardy--Weinberg equilibrium, the overall recessive genotype probability is
$P_{\text{AR}} = P_{\text{tot}}^2
= \sum_i p_i^2 + 2\sum_{i<j} p_i p_j$.
To partition this gene-level risk without double counting compound heterozygotes, we allocate risk proportionally by
\(p_{\text{disease},i} = p_i P_{\text{tot}}\),
which satisfies
\(\sum_i p_{\text{disease},i} = P_{\text{tot}}^2\).

More simply, dominant and X-linked conditions scale with the allele frequency of each variant, whereas recessive conditions scale with the total pathogenic allele frequency in the gene.

\paragraph{Expected case numbers and case detection probability.}
...
\paragraph{Aggregation of variant-level estimates.}
All quantities were defined at the variant level. 
For reporting and comparison purposes, we aggregated these variant-level results by gene and by ClinVar classification (e.g. ``Pathogenic'', ``Likely pathogenic'', ``Uncertain significance''). 
Alternative scoring systems can be substituted without altering the underlying calculations.
The total expected case count for a group was 
$E_{\text{group}} = \sum_{i \in \text{group}} E_i$.
Assuming independence between variants, the probability of observing at least one case within a group was
$P_{\text{group}} = 1 - \prod_{i \in \text{group}} \left(1 - p_{\text{disease},i}\right)$.



\subsection{Classification scoring summaries}
\paragraph{Score-positive-total.}
...
\paragraph{Classification scoring system.}
...
\paragraph{Data processing and implementation.}
...
\acro{gvcf}[gVCF]{genomic variant call format}
 ...
 \acro{nfkb1}[NFKB1]{Nuclear Factor Kappa B Subunit 1}
...
\subsection{Methods}
\subsubsection*{Validation of autosomal dominant estimates using \textit{NFKB1}}
...
\subsubsection*{Validation study for autosomal recessive CF using \textit{CFTR}}
...
\subsubsection*{Validation of SCID-specific estimates using PID-SCID genes}
...
\subsection{Results}
\subsubsection*{Occurrence probability across disease genes}
...
\clearpage
\subsubsection*{Integrating observed true positives and unobserved false negatives into a single, actionable conclusion}
...
\subsubsection*{Scenario three - currently impossible diagnosis}
...
\subsubsection*{Observed variant likelihood is independent of AlphaMissense pathogenicity}
