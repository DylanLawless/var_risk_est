...
header_line <- readLines("../data/tnfaip3_head", n = 1)
header_line <- sub("^#", "", header_line)
header_fields <- strsplit(header_line, "\t")[[1]]
rm(header_line)

df <- read.table("../data/tnfaip3", 
                 sep = "\t",
                 header = FALSE, 
...
p_count <- ggplot(df, aes(
  ...
  guides(fill = "none")
p_count
ggsave("../images/tnfaip3_clinvar_count.png", plot = p_count, width = 5, height = 4)
...
#             vjust = 3, hjust = 0, color = "black", size = 3) +
...
print(p_scatter)
ggsave("../images/tnfaip3_scatter_expected_prob.png", plot = p_scatter, width = 5, height = 6)
...
p_bars <- (p_bar / p_prob) + plot_layout(guides = 'collect', axis = "collect")  + plot_annotation(tag_levels = 'A')
print(p_bars)
ggsave("../images/tnfaip3_combined_bar_charts.png", plot = p_bars, width = 6, height = 8)
